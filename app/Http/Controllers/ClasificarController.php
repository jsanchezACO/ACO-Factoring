...
class ClasificarController extends Controller
{
    ...
    public function Clasifica(Request $request)
    {
        try {
            ...
            $NOTAS = $this->completaCampo('Reclacificacion Tercero plataforma factoring', 255, 'ALFA'); //Observaciones del documento            
            ...
            $Clasificar = $request->Factura;
            foreach ($Clasificar as $Clasifica) {
                ...
                $CONSEC_DOCTO_CRUCE = $this->completaCampo($Facturas->Factura, 8, 'NUM'); //Número de documento de cruce
                $FECHA_VCTO = $this->completaCampo($Clasifica['Vencimiento'], 8, 'ALFA'); //
                $FECHA_DOCTO_CRUCE = $this->completaCampo($Facturas->FechaFactura, 8, 'ALFA'); //
                $FECHA_DSCTO_PP = $this->completaCampo($Facturas->PP, 8, 'ALFA'); // 
                $VALOR_IMP_ASUM = $this->formatoSigno($Facturas->Impuesto, 15);    //Valor impuesto asumido
                $VALOR_IMP_ASUM2 = $this->formatoSigno($Facturas->Impuesto, 15);    //Valor impuesto asumido  

                //Banco    
                $ID_TERCERO = $this->completaCampo($request->Banco, 15, 'ALFA'); //
                $VALOR_CR = $this->formatoSigno($Facturas->ValorFactura, 15);    //Valor de factura a pagar
                $VALOR_CR2 = $this->formatoSigno($Facturas->ValorFactura, 15);    //Valor de factura a pagar              
                ...
                $VALOR_DB = $this->formatoSigno($Facturas->ValorFactura, 15);    //Valor debito se deja en cero
                $VALOR_DB2 = $this->formatoSigno($Facturas->ValorFactura, 15);    //Valor debito                
